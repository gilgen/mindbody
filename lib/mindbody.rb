require 'savon'

module MindBody
end

require "mindbody/version"
require 'mindbody/api_error'
require 'mindbody/config'
require 'mindbody/result'
require 'mindbody/soap'
require 'mindbody/models/session_type'
require 'mindbody/service'
require 'mindbody/appointment_service'
require 'mindbody/class_service'
require 'mindbody/client_service'
require 'mindbody/sale_service'
require 'mindbody/site_service'
require 'mindbody/staff_service'
require 'mindbody/purchase'
require 'mindbody/payments'
require 'mindbody/payments/comped'
require 'mindbody/payments/credit_card'
require 'mindbody/payments/stored_credit_card'
